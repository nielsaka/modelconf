...
y_l0 <- embed(y, 2)[, 1]
y_l1 <- embed(y, 2)[, 2]
lm(y_l0 ~ y_l1) # makes any sense?

# Time-trend -> will be intercept of change? indeed.
lm(log(DAX) ~ seq_along(DAX))
lm(y ~ 1)

plot(log(DAX), ylim = c(7, 9))
# abline(lm(log(DAX) ~ I(seq_along(DAX)/4 + 1992)))
# abline(lm(log(DAX) ~ seq_along(DAX)))
cf = lm(log(DAX) ~ seq_along(DAX))$coef

a = cf[1] - cf[2]*400 * (1991.496+0.25)
b = cf[2] * 400

abline(h = cf[1])
abline(a = a, b = b)

# argh..., scrap the time dimension
ldax <- log(DAX)
attributes(ldax) <- NULL

plot(ldax, type = "l")
abline(lm(ldax ~ seq_along(ldax))) # looks really off, but just for illustration
